...
export default function SetlistPlanner({ songs, setlists, onUpdateSetlists, onUpdateSong, onSaveProgram }) {
  ...
  const setEventName = (eventName) => {
    if (permanentIds.includes(activeEvent?.id)) return;
    updateActive({ eventName });
  };
  ...
  const deleteEvent = (id) => {
    if (permanentIds.includes(id)) return;
  ...
  return (
    <div className="pb-24 px-4 pt-2 max-w-md mx-auto">
      ...
      <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-1 no-scrollbar">
        {setlists.map((ev) => {
          const isPermanent = permanentIds.includes(ev.id);
          return (
        ...
        {addingEvent ? (
          <div className="flex items-center gap-1 shrink-0">
            ...
            />
            <button onClick={addEvent} className="text-sm px-2 font-bold" style={{ color: theme.accent }}>
              OK
            </button>
            <button onClick={() => setAddingEvent(false)} className="text-sm px-1" style={{ color: theme.muted }}>
              ✕
            </button>
          </div>
        ) : (
      ...
      <div className="rounded-2xl p-4 border mb-4 shadow-lg" style={{ backgroundColor: theme.surface, borderColor: theme.border }}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <input
            ...
            disabled={permanentIds.includes(activeEvent?.id)}
            className="bg-transparent font-bold focus:outline-none w-full text-lg disabled:opacity-70"
            style={{ color: theme.text }}
          />
        </div>
        <div
          className="flex items-center gap-2 mb-4 rounded-xl px-3 py-2.5"
          style={{ backgroundColor: theme.bg, border: `1px solid ${theme.border}` }}
        >
          <span className="text-xs font-semibold shrink-0" style={{ color: theme.muted }}>
            Data programului:
          </span>
          <input
            type="date"
          ...
          return (
            <div key={index} className="rounded-xl p-3 border shadow-sm" style={{ backgroundColor: theme.surface, borderColor: theme.border }}>
              <div className="flex items-center gap-2">
                ...
                {!isOriginal && <span className="text-xs shrink-0" style={{ color: '#fb7185' }}>→{item.originalKey}</span>}
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    onClick={() => setAranjamentIndex(index)}
                    className="p-1.5 rounded-lg active:scale-90 transition relative"
                    style={{
                      backgroundColor: item.aranjament?.voceaLead ? theme.accent + '22' : theme.bg,
                      color: item.aranjament?.voceaLead ? theme.accent : theme.muted,
                    }}
                  >
      ...
      {/* ── Aranjament modal ── */}
      {aranjamentSong && (
        <div className="fixed inset-0 z-50 flex items-end anim-overlay" style={{ backgroundColor: 'rgba(0,0,0,0.65)' }} onClick={() => setAranjamentIndex(null)}>
          <div
            className="w-full rounded-t-3xl max-h-[85vh] flex flex-col anim-sheet"
